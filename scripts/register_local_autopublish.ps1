...
if ($WorkbookPath) {
    ...
    foreach ($root in $searchRoots) {
        $preferred = Get-ChildItem -Path $root -File -ErrorAction SilentlyContinue | Where-Object {
            $_.Extension -in ".xlsx", ".xlsm" -and $_.Name -like "*Liseo Assemblies Master Sheet*"
        } | Select-Object -First 1
        if (-not $preferred) {
            $preferred = Get-ChildItem -Path $root -File -ErrorAction SilentlyContinue | Where-Object {
                $_.Extension -in ".xlsx", ".xlsm"
            } | Select-Object -First 1
        }
        if ($preferred) {
